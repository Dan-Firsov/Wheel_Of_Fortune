...
export default function GameSessionPanel() {
  const { gamePhase, timeLeft, newGameTimeLeft, winner, winnerPrize } =
    useGameSessionPanel();
    ...
    <div className={styles.gameSessionPanelWrapper}>
      ...
      {gamePhase === GamePhase.ONGOING && timeLeft !== null && (
        <div>
          ...
          {timeLeft && (
            <progress
              className={`${styles.progressbarGameSession} animated"`}
              id='p1'
              value={timeLeft}
              max='60'
            ></progress>
      ...
      {gamePhase === GamePhase.DETERMINING && (
        <h2>Determining the winner...</h2>
      )}

      {gamePhase === GamePhase.NEXTGAMETIMER && newGameTimeLeft !== null && (
        <h2>Next game starts in: {newGameTimeLeft} sec</h2>
      )}

      {winner && (
        <h3>
          Winner: {winner?.slice(0, 7) + '....' + winner?.slice(-6)} Winning
          prize: {winnerPrize} ETH
        </h3>
      )}
    </div>
  );
